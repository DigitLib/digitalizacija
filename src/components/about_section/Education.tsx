...
export default function Education(props: EducationProps) {
  ...
  return education.length == 0 ? (
    ...
    <div className="flex flex-col space-y-4 lg:w-3/4 mx-4">
      <h1 className="text-3xl font-bold">Циљ</h1>
      {education.map((edu, index) => (
        <div className="flex flex-col space-y-2 relative" key={index}>
          ...
          <div className="relative left-10 w-full">
            ...
            {edu.gpa && (
              <p className="text-xl font-normal">Подршка: {edu.gpa}</p>
            )}
            {edu.thesis && (
              <p className="text-xl font-normal">Пројекат: {edu.thesis}</p>
            )}
            {edu.realisation && (
              <p className="text-xl font-normal">
                Реализација: {edu.realisation}
              </p>
            )}
          </div>
